...
import {
  CHAIN_ID, CONTRACT_ADDRESS, DEPLOYMENT_TX, SOURCE_SHA, type Compatibility, type Importance, type Negotiation, type Party, type PositionTerm, type Synthesis,
  contractExplorerUrl, getWalletAddress, readNegotiation, readSynthesis, requestWallet, switchToStudioDev, watchWallet, writeMethod
} from "./genlayer";
  ...
  return <main>
    <section className="hero section">
      ...
      <div className="hero-grid">
        ...
        <div className="hero-copy"><p className="eyebrow">A semantic protocol for two parties</p><h1>Agreement,<br /><em>made explicit.</em></h1><p className="hero-lede">Handshake turns two immutable positions into a proposed agreement grounded in both sources. GenLayer synthesizes the overlap; both parties decide whether to seal it.</p><div className="hero-actions"><Link className="button button-ink" to="/app/new">Start a negotiation <ArrowRight size={15} /></Link><Link className="text-link" to="/compare">Read the compatibility model</Link></div></div>
      </div>
      <div className="hero-foot"><span>01 / DECLARE</span><span>02 / SYNTHESIZE</span><span>03 / ACCEPT</span><span>04 / SEAL</span></div>
    </section>
...
function Workspace() {
  const [id, setId] = useState("handshake-live-compatible-20260925-r5");
...
function Detail() {
  ...
  const [error, setError] = useState("");
  const load = async () => { try { setError(""); const next = await readNegotiation(id); setRecord(next); if (next.synthesis_fingerprint) setSynthesis(await readSynthesis(id)); } catch (e) { setError(errorText(e)); } };
...
function Compare() {
  return <main className="section compare"><div className="section-index">COMPATIBILITY / A SHARED VOCABULARY</div><div className="compare-head"><p className="eyebrow">REALISTIC TEST SCENARIO</p><h1>Where positions<br /><em>meet.</em></h1><p>Compatibility is not a score. It is a grounded statement about what can be proposed without weakening what either party marked HARD.</p></div><div className="compare-table"><div className="compare-labels"><span>TERM DOMAIN</span><span>PARTY A</span><span>PARTY B</span><span>READING</span></div>{[["PAYMENT","min $2,000 / HARD","max $2,500 / HARD","compatible range"],["DELIVERY","within 14 days / HARD","within 21 days / HARD","14-day floor"],["REVISIONS","two rounds / HARD","at least two / HARD","aligned"],["PAYMENT TIMING","50% upfront / PREFERENCE","milestones / PREFERENCE","unresolved preference"]].map((row) => <div className="compare-row" key={row[0]}>{row.map((value, i) => i === 0 ? <strong key={value}>{value}</strong> : <span key={value} className={i === 3 ? "reading" : ""}>{value}</span>)}</div>)}</div><div className="compatibility-note"><span>COMPATIBLE != AUTOMATICALLY SEALED</span><p>A compatible proposal still needs the exact synthesis fingerprint accepted by both distinct parties.</p></div><div className="compare-split"><div><h2>Hard constraints<br /><em>stay hard.</em></h2><p>Conflicting HARD payment or ownership terms cannot be returned as fully COMPATIBLE. Preferences may remain unresolved, but they never override a requirement.</p></div><div className="conflict-card"><span>IRRECONCILABLE EXAMPLE</span><strong>A: minimum $5,000</strong><strong>B: maximum $2,000</strong><b>INCOMPATIBLE</b></div></div></main>;
}
